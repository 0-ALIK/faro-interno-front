...
@Component({
  selector: 'app-type-libraries-list',
  ...
  template: `
    ...
    <div class="flex flex-col gap-6">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-h2">Tipos de biblioteca</h1>
          <p class="text-body text-muted-color">Gestiona los tipos de biblioteca del catálogo.</p>
        </div>
        <p-button label="Nuevo tipo" icon="pi pi-plus" (onClick)="openCreate()" />
      </div>
      ...
      <div class="flex flex-col gap-3 rounded-2xl border border-surface-200 bg-surface-0 p-4 shadow-sm md:flex-row md:items-center md:justify-between">
        <p-iconfield class="w-full md:w-80">
          ...
          <input pInputText type="search" [value]="search()" (input)="onSearchInput($event)" placeholder="Buscar tipo…" class="w-full" />
        </p-iconfield>
      ...
      <div class="overflow-hidden rounded-2xl border border-surface-200 bg-surface-0 shadow-sm">
        <p-table [value]="items()" [loading]="cultureState.loading()" dataKey="id" [tableStyle]="{ 'min-width': '40rem' }">
          ...
          <ng-template pTemplate="emptydata">
            <tr><td colspan="2" class="py-10 text-center text-muted-color">No hay tipos de biblioteca registrados.</td></tr>
          </ng-template>
        </p-table>
...
export class TypeLibrariesList implements OnInit {
  protected readonly cultureState = inject(CultureStateService);
  private readonly messageService = inject(MessageService);

  protected readonly saving = signal(false);
  protected readonly dialogVisible = signal(false);
  protected readonly dialogHeader = signal('Crear tipo');
  ...
  private async loadData(): Promise<void> {
    const result = await this.cultureState.getTypeLibrariesPage(this.page(), this.search());
  ...
  protected openCreate(): void {
    this.editingId = null;
    this.dialogHeader.set('Crear tipo');
  ...
  protected openRename(item: { id: string; name: string }): void {
    ...
    this.dialogHeader.set('Renombrar tipo');
  ...
  protected async onDelete(id: string): Promise<void> {
    await this.cultureState.deleteTypeLibrary(id);
  ...
  protected async onSave(name: string): Promise<void> {
    ...
    try {
      if (this.editingId) {
        await this.cultureState.renameTypeLibrary(this.editingId, name);
        this.messageService.add({ severity: 'success', summary: 'Actualizado', detail: 'Tipo renombrado.' });
      } else {
        await this.cultureState.createTypeLibrary(name);
        this.messageService.add({ severity: 'success', summary: 'Creado', detail: 'Nuevo tipo agregado.' });
      }
      this.closeDialog();
      void this.loadData();
    } finally {
