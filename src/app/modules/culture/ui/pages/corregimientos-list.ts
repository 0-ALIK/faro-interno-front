...
@Component({
  selector: 'app-corregimientos-list',
  ...
  template: `
    ...
    <div class="flex flex-col gap-6">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-h2">Corregimientos</h1>
          <p class="text-body text-muted-color">Gestiona los corregimientos del municipio.</p>
        </div>
        <p-button label="Nuevo corregimiento" icon="pi pi-plus" (onClick)="openCreate()" />
      </div>
      ...
      <div class="flex flex-col gap-3 rounded-2xl border border-surface-200 bg-surface-0 p-4 shadow-sm md:flex-row md:items-center md:justify-between">
        <p-iconfield class="w-full md:w-80">
          ...
          <input pInputText type="search" [value]="search()" (input)="onSearchInput($event)" placeholder="Buscar corregimiento…" class="w-full" />
        </p-iconfield>
      ...
      <div class="overflow-hidden rounded-2xl border border-surface-200 bg-surface-0 shadow-sm">
        <p-table [value]="items()" [loading]="cultureState.loading()" dataKey="id" [tableStyle]="{ 'min-width': '40rem' }">
          ...
          <ng-template pTemplate="emptydata">
            <tr><td colspan="2" class="py-10 text-center text-muted-color">No hay corregimientos registrados.</td></tr>
          </ng-template>
        </p-table>
    ...
    <app-entity-dialog [visible]="dialogVisible()" [header]="'Crear corregimiento'" [initialValue]="''" [saving]="saving()" (save)="onSave($event)" (close)="dialogVisible.set(false)" />
  `
})
export class CorregimientosList implements OnInit {
  ...
  private async loadData(): Promise<void> {
    const result = await this.cultureState.getCorregimientosPage(this.page(), this.search());
  ...
  protected async onDelete(id: string): Promise<void> {
    await this.cultureState.deleteCorregimiento(id);
    this.messageService.add({ severity: 'success', summary: 'Eliminado', detail: 'Corregimiento eliminado.' });
    void this.loadData();
  }
  ...
  protected async onSave(name: string): Promise<void> {
    ...
    try {
      await this.cultureState.createCorregimiento(name);
      this.messageService.add({ severity: 'success', summary: 'Creado', detail: 'Nuevo corregimiento agregado.' });
