...
@Component({
  selector: 'app-category-activities-list',
  ...
  template: `
    ...
    <div class="flex flex-col gap-6">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-h2">Categorías de actividad</h1>
          <p class="text-body text-muted-color">Gestiona las categorías de actividad del catálogo.</p>
        </div>
      ...
      <div class="flex flex-col gap-3 rounded-2xl border border-surface-200 bg-surface-0 p-4 shadow-sm md:flex-row md:items-center md:justify-between">
        <p-iconfield class="w-full md:w-80">
          ...
          <input pInputText type="search" [value]="search()" (input)="onSearchInput($event)" placeholder="Buscar categoría…" class="w-full" />
        </p-iconfield>
      ...
      <div class="overflow-hidden rounded-2xl border border-surface-200 bg-surface-0 shadow-sm">
        <p-table [value]="items()" [loading]="cultureState.loading()" dataKey="id" [tableStyle]="{ 'min-width': '40rem' }">
          ...
          <ng-template pTemplate="emptydata">
            <tr><td colspan="2" class="py-10 text-center text-muted-color">No hay categorías de actividad registradas.</td></tr>
          </ng-template>
        </p-table>
...
export class CategoryActivitiesList implements OnInit {
  ...
  private async loadData(): Promise<void> {
    const result = await this.cultureState.getCategoryActivitiesPage(this.page(), this.search());
  ...
  protected async onDelete(id: string): Promise<void> {
    await this.cultureState.deleteCategoryActivity(id);
  ...
  protected async onSave(name: string): Promise<void> {
    ...
    try {
      if (this.editingId) {
        await this.cultureState.renameCategoryActivity(this.editingId, name);
        this.messageService.add({ severity: 'success', summary: 'Actualizada', detail: 'Categoría renombrada.' });
      } else {
        await this.cultureState.createCategoryActivity(name);
