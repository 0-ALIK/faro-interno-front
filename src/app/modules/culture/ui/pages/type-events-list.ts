  ...
  template: `
    ...
    <div class="flex flex-col gap-6">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-h2">Tipos de evento</h1>
          <p class="text-body text-muted-color">Gestiona los tipos de evento del catálogo.</p>
        </div>
      ...
      <div class="overflow-hidden rounded-2xl border border-surface-200 bg-surface-0 shadow-sm">
        <p-table [value]="items()" [loading]="cultureState.loading()" dataKey="id" [tableStyle]="{ 'min-width': '40rem' }">
          ...
          <ng-template pTemplate="emptydata">
            <tr><td colspan="2" class="py-10 text-center text-muted-color">No hay tipos de evento registrados.</td></tr>
          </ng-template>
        </p-table>
...
export class TypeEventsList implements OnInit {
  ...
  private async loadData(): Promise<void> {
    const result = await this.cultureState.getTypeEventsPage(this.page(), this.search());
  ...
  protected async onDelete(id: string): Promise<void> {
    await this.cultureState.deleteTypeEvent(id);
  ...
  protected async onSave(name: string): Promise<void> {
    ...
    try {
      if (this.editingId) {
        await this.cultureState.renameTypeEvent(this.editingId, name);
        this.messageService.add({ severity: 'success', summary: 'Actualizado', detail: 'Tipo renombrado.' });
      } else {
        await this.cultureState.createTypeEvent(name);
